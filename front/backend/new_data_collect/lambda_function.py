...
def lambda_handler(event, context):
    """Lambda 핸들러: 뉴스 데이터 분석 및 API Gateway 응답"""
    
    logger.info(f"Lambda 이벤트: {json.dumps(event, ensure_ascii=False)}")
    ...
    # content_list 엔드포인트 처리
    if path == '/content/content_list' and http_method == 'GET':
        return handle_content_list(query_params)
    
    # 기존 뉴스 수집 로직 (key 파라미터 기반)
    # 파라미터 검증
    logger.info("파라미터 검증 시작")
    
    # key 파라미터 확인 (쿼리 파라미터 또는 이벤트에서)
    key_value = query_params.get('key') or event.get('key')
    if not key_value:
    ...
    if key_value != 'group01':
        ...
        return create_api_response(403, {
            'message': '잘못된 key 값',
...
def handle_content_list(query_params):
    """content_list 엔드포인트 처리"""
    logger.info("content_list 엔드포인트 호출")
    
    # 여기에 content_list 로직 구현
    # 현재는 기본 응답만 반환
